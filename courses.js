...
for(let course of courses) {
    if(typeof(course) === 'string') {
        ...
        str += `<${typeof(course[course.length - 1]) === 'number' ? 'section' : 'div'} id="${course[1]}" class="course" onmouseleave="Remove_highlights()" onmouseover="get_requirements(this)"><div>
        ${course[0]}</div><div>${course[1]}</div>${course[3] ? `<div class="note">${course[3]}</div>`:''}</${typeof(course[course.length - 1]) === 'number' ? 'section' : 'div'}>`;
    }
}
